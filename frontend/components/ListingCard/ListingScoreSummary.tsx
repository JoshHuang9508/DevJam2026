...
export function ListingScoreSummary({ listing, rank }: { listing: ScoredListing; rank: number }) {
  return (
    <>
      ...
      <h2 className="mt-1 break-words text-base font-semibold leading-snug text-neutral-900">{listing.title}</h2>
      <div className="mt-3 border-t border-neutral-100 pt-3">
        <p className="text-xs text-neutral-500">整體評分</p>
        <div className="mt-1"><StarRating score={listing.score} size="large" /></div>
      </div>
      <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 border-t border-neutral-100 pt-3">
        {WEIGHT_KEYS.map((key) => (
          <div key={key} className="min-w-0">
            <dt className="text-[11px] text-neutral-500">{WEIGHT_LABELS[key]}</dt>
            <dd className="text-[13px] leading-tight text-neutral-900">
