...
type Row = { city: string; district: string; price: number; area: number }
 ...
 */
export async function datasetSummary(mode: Mode): Promise<DatasetSummary> {
  const all = await listingRows(mode)
 ...
 */
export async function areaCoverageNote(mode: Mode, hard: HardConstraints): Promise<string | null> {
  const named = [...(hard.districts ?? []), ...(hard.cities ?? []), ...(hard.regions ?? [])]
  if (named.length === 0) return null
  const matched = (await listingRows(mode)).filter((r) => !areaExcludes(r, hard))
  if (matched.length > 0) return null
  return `物件資料集目前沒有${named.join('、')}的資料（涵蓋範圍：${
    (await datasetSummary(mode)).cities.join('、') || '無'
  }），所以這個地區查不到物件。地區條件不會自動放寬。`
}

export { listingsDbAvailable } from './listing-data'
