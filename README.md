...
這是本地展示用途的專案，**不要直接部署到公開網路**。`/api/chat` 與 `/api/rank` 兩條路由都沒有身分驗證、
沒有速率限制，`request.json()` 也沒有限制請求大小上限。`/api/chat` 每次請求會呼叫 Gemini 兩次
（萃取一次、生成解釋文字一次）——公開曝露等於讓任何人都能免費消耗你的 API 額度與伺服器記憶體，
形成成本與記憶體的阻斷服務風險。本地開發、demo 沒有問題；若要對外提供服務，至少需要加上身分驗證、
速率限制與請求大小限制。
